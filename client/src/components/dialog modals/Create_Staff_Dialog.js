...
function Create_Staff_Dialog({ openCreateStaff, closeCreateStaff }) {
    ...
    const handleSubmit = async(e) => {
        ...
        if(accountDetails.password !== accountDetails.confirmPassword){
            ...
            const isEmailRegisteredRes = await isEmailRegistered({email: accountDetails.email})
            if(isEmailRegisteredRes){
                if(isEmailRegisteredRes.status === 200){
                    ...
                    if(data.exist === true){
                        ...
                        const res = await RegisterStaff({ email: accountDetails.email, password: accountDetails.password, role: accountDetails.role })
            ...
            >
                        ...
                        <div className="Inputs">
                            ...
                            <div className="Radio_Container">
                                <div className="radio-input">
                                    <label>
                                        <input type="radio" id="value-1" name="value-radio" onClick={() => setAccountDetails({...accountDetails, role: 'Secretary'})} checked={accountDetails.role === "Secretary"}value="Secretary" disabled={submit}/>
                                        <span>Secretary</span>
                                    </label>
                                    <label>
                                        <input type="radio" id="value-2" name="value-radio" onClick={() => setAccountDetails({...accountDetails, role: 'Clerk'})} checked={accountDetails.role === "Clerk"} value="Clerk" disabled={submit}/>
                                        <span>Clerk</span>
                                    </label>
                                    <label>
                                        <input type="radio" id="value-3" name="value-radio" onClick={() => setAccountDetails({...accountDetails, role: 'SA'})} checked={accountDetails.role === "SA"} value="SA" disabled={submit}/>
                ...
                <DialogActions>
                    <div className="Dialog_Actions">
                        ...
                        <button type='submit' form='createStaff_Form' className='Dialog_Submit'>
