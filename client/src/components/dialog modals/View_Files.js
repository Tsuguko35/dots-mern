...
function View_Files({
  ...
  return (
    <section id="View_Files" className="View_Files">
      {/* View PDF */}
      {pdfToView && <ViewPdf pdfFile={pdfToView} closePDf={setPdfToView} />}
      <div className="wrapper">
        <TabContext value={value}>
          ...
          >
            ...
            {files.some((file) => /\.(pdf)$/i.test(file.file_Name)) && (
              <Tab label="PDF" value="2" />
            )}
            {files.some((file) => /\.(doc|docx)$/i.test(file.file_Name)) && (
              <Tab label="Docx" value="3" />
            )}
            {files.some((file) => /\.(xls|xlsx)$/i.test(file.file_Name)) && (
                ...
                <ImageList cols={imageCols} gap={8}>
                  {imageFiles &&
                    imageFiles.map((img, index) => (
                      <ImageListItem
                        className="Image_Item_Holder"
                        key={img.file_id}
                        onClick={() => openLightbox(index)}
                      >
                        <img
                          srcSet={`${domain}${documentFiles}/${
                            img.document_id || img.archive_id
                          }-${
                            img.file_Name
                          }?w=248&fit=crop&auto=format&dpr=2 2x`}
                          src={`${domain}${documentFiles}/${
                            img.document_id || img.archive_id
                          }-${img.file_Name}?w=248&fit=crop&auto=format`}
                          alt={img.file_Name}
                          ...
                          <div className="Actions">
                            {windowWidth >= 768 && (
                              <div
                                ...
                                onClick={() =>
                                  setPdfToView(
                                    `${domain}${documentFiles}/${
                                      pdf.document_id || pdf.archive_id
                                    }-${encodeURIComponent(pdf.file_Name)}`
                                  )
                                }
                              >
                          ...
                          <div className="Actions">
                            <div
                              ...
                              onClick={() =>
                                handleDownload({
                                  document_id:
                                    doc.document_id || doc.archive_id,
                          ...
                          <div className="Actions">
                            <div
                              ...
                              onClick={() =>
                                handleDownload({
                                  document_id:
                                    xlsx.document_id || xlsx.archive_id,
