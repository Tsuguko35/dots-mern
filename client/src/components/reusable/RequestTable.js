...
function RequestTable({documentType, requestType, documents, filters, setFilter, getTableDcuments, userDetails, isLoading, trackers, refreshTracker}) {
                        ...
                        {documents && documents.length !== 0 ? (
                          <React.Fragment>
                            {documents.map((document) => (
                                <div key={document.document_id} className={`Table_Body_Row ${(notifications.find(notif => notif.document_id === document.document_id)?.isRead === 0 && requestType !== 'History') && 'unread'} ${(parseInt(document.urgent) === 1 && requestType !== 'History' && requestType !== 'Approved' && requestType !== 'Rejected') && 'urgent'}`} onClick={() => removeNotification(document.document_id)}>
                                            ...
                                            (
                                              ...
                                              {trackers && trackers.filter(tracker => tracker.document_id === document.document_id).map((tracker) => (
                                                <div className="Tracker_Item" key={tracker.tracker_id}>
                                                  ...
                                                  {trackers.filter(tracker => tracker.document_id === document.document_id).length > 1 && (
