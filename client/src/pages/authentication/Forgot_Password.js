...
function Forgot_Password() {
        ...
        if(email){
            ...
            if(emailFormat.test(email)){
                if(email.endsWith('@bulsu.edu.ph')){
                    ...
                    //Send OTP
                    const otpRes = await otpRequest({ action: "Registration", receiver: email })
                    ...
                    setError({ isError: true, errorMessage:'Please enter a BulSU email.' })
                }
            }
            else{
                setError({ isError: true, errorMessage:'Please enter a valid email address.' })
            }
        }   
    }
    ...
    const resetPassword = async() => {
        ...
        if(passwords.password === passwords.confirmPassword){
            console.log(true);
            const resetRes = await resetUserPassword({ email: email, password: passwords.password })
                                    ...
                                    <div className="Forgot_Password_Input">
                                        <div className="Input_Group">
                                            ...
                                            <div className={!error.isError ? "Custom_Email" : "Custom_Email error"}>
                                                <input className='Input' type="email" placeholder='Email Address' onChange={(e) => setEmail(e.target.value)}/>
                                            </div>
                                        </div>
                                    </div>
                                    {error.isError && (
                        ...
                        :activeStep === 1 ?(
                            <React.Fragment>
                                <div className="Forgot_Password_Label">
                                    ...
                                    <div className="Sub_Label">
                                        <p>The OTP code has been sent to {email}. Enter the OTP code to proceed.</p>
                                    </div>
                                </div>
                                <div className="Forgot_Password_Content_Inner">
                                    <div className="Forgot_Password_Input">
