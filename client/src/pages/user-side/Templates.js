...
import { domain, template_File } from '../../constants'
...
function Templates() {
  ...
  const handleDeleteTemplate = (template_id, date_Added, file_Name) => {
    Swal.fire({
        icon: 'question',
        iconColor: '#FF8911',
    ...
    if (result.isConfirmed) {
        const res = await deleteTemplate({ template_id: template_id, date_Added: date_Added, file_Name: file_Name })
        if(res?.status === 200){
  ...
  useEffect(() => {
    if(templatesToFilter && templatesToFilter.length > 0){
      const filteredTemplate = templatesToFilter.filter(templates => 
        templates.date_Added.includes(filters.year)
      )
      .filter(templates => 
  ...
  const handleDownload = async(props) => {
    ...
    const url = `${domain}${template_File}/${props.date_Added}-${props.template_Name}`;
  ...
  return (
    <section id='Templates' className='Templates'>
        {/* Template Add Dialog */}
        <Add_Template_Dialog openAddTemplate={openAddTemplate} closeAddTemplate={setOpenAddTemplate} getTemplates={getTemplates}/>
              ...
              <div className="Search_Template">
                ...
                <div className="Input_Group">
                    <div className="Custom_Search">
                        ...
                        <input className='Input' type="text" placeholder='Search...' onChange={(e) => setFilters({...filters, search: e.target.value})}/>
                    </div>
                </div>
              </div>
            </div>
                      ...
                      <div className="Template_Item" key={template.template_id}>
                        <div className="Template_Icon">
                          <div className="Icon">
                            ...
                            :(template.template_Name.endsWith('.doc') || template.template_Name.endsWith('.docx')) ? (
                                <DOCX />
                            )   
                            :(template.template_Name.endsWith('.xls') || template.template_Name.endsWith('.xlsx')) && (
                                <XLSX />
                            )
                            }
                          </div>
                        ...
                        <div className="Template_Bottom">
                          <div className="Template_Label">
                            ...
                            <p className="Year">{template.date_Added.substring(0, 4)}</p>
                          </div>
                          <div className="Template_Actions">
                            <button onClick={() => handleDownload({ date_Added: template.date_Added, template_Name: template.template_Name })}>Download</button>

                            <span onClick={() => handleDeleteTemplate(template.template_id, template.date_Added, template.template_Name)}>Delete</span>
                          </div>
                        </div>
                      </div>
                    ))}
